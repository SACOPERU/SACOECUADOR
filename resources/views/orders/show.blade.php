...
<script src="https://cdn.jsdelivr.net/npm/sweetalert2@10"></script> <!-- Agrega el script de SweetAlert -->

@if ($order->status == 2)
    <script>
        // Verificar si la alerta ya ha sido mostrada para este pedido
        let orderAlertKey = 'orderAlert_{{ $order->id }}';
        let showAlert = localStorage.getItem(orderAlertKey);
        ...
        if (!showAlert) {
            ...
            // Marcar la alerta como mostrada para este pedido
            localStorage.setItem(orderAlertKey, true);
        }
    </script>
@endif
