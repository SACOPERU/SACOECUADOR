    ...
    <section class="my-2 container">
        ...
        <div id="default-carousel" class="relative w-full" data-carousel="slide">
            ...
            <div class="relative h-56 overflow-hidden rounded-lg md:h-96">
                @foreach ($scovers as $index => $scover)
                    @if ($isMobile && Str::contains(strtolower($scover->name), 'movil'))
                        <div class="carousel-item {{ $index === 0 ? 'active' : '' }}">
                            <img src="{{ asset('storage/' . $scover->image) }}"
                                class="absolute block w-full h-full object-cover" alt="...">
                        </div>
                    @elseif (!$isMobile && Str::contains(strtolower($scover->name), 'banners'))
                        <div class="carousel-item {{ $index === 0 ? 'active' : '' }}">
                            <img src="{{ asset('storage/' . $scover->image) }}"
                                class="absolute block w-full h-full object-cover" alt="...">
                        </div>
