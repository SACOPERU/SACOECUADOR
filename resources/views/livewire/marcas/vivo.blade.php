<header class="bg-gray-100 sticky top-0 z-50 border-b-2 border-blue-400" x-data="dropdown()">
    <div class="container flex items-center h-16 justify-between md:justify-start space-x-8">

        <a href="/" class="mx-6">
            <img class=" navbar-brand-full app-header-logo" src="{{ asset('img/Nueva carpeta/Vivo.png') }}" width="130"
                alt="Infyom Logo">
        </a>
        ...
        <div x-data="{ activeCategory: null, closeTimer: null, isWindowWideEnough: window.innerWidth >= 1300 && window.innerHeight >= 600 }" x-init="() => {
            ...
            @foreach ($categories as $category)
                <div class="relative inline-block group"
                    ...
                    <x-jet-nav-link :class="'hover:border-blue-400 hover:text-blue-400'" href="{{ route('categories.show', $category) }}"
                        :active="''">
                        {{ $category->name }}
                    </x-jet-nav-link>
                    ...
                    <div x-show="activeCategory == '{{ $category->id }}'" class="fixed top-0 left-0 z-0 mt-16">
                        <div class="flex flex-row">
                            <div class="w-screen bg-gray-100 p-10 border border-blue-400">
